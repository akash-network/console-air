...
export class NvidiaGpuService {
  constructor(
    ...
    private readonly logger: LoggerService = new LoggerService({ name: "NvidiaGpuService" })
  ) {}
...
/**
 * Extracts the overall EAT (JWT) from an NRAS V3 response. Confirmed live shape is a tuple
 * `[["JWT", overallEat], { "GPU-0": <perDeviceEat>, ... }]`; we also tolerate a bare token string or a
 * `[overallEat, ...]` tuple for forward/backward compatibility.
 */
