...
describe(AmdKdsClient.name, () => {
  ...
  describe("getCaChain", () => {
    ...
    it("caches the chain so a repeat lookup skips the HTTP fetch", async () => {
      const { client, httpClient } = setup();
      httpClient.get.mockResolvedValue({ data: `${CERT_ASK}\n${CERT_ARK}` });

      await client.getCaChain("Genoa");
      await client.getCaChain("Genoa");
  ...
  describe("getCrl", () => {
    ...
    it("caches the CRL so a repeat lookup skips the HTTP fetch", async () => {
      const { client, httpClient } = setup();
