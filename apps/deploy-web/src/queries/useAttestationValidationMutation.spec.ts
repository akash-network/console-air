...
import { ATTESTATION_VALIDATE_PATH, useAttestationValidationMutation } from "./useAttestationValidationMutation";

import { setupQuery } from "@tests/unit/query-client";

describe(useAttestationValidationMutation.name, () => {
  it("posts the evidence to the local validation route and exposes the per-report verdicts", async () => {
    const verdicts: AttestationValidationResult = {
      overall: "valid",
      nonce: "nonce-base64",
      reports: [{ kind: "cpu", vendor: "amd-sev-snp", status: "valid", detail: "ok" }]
    };
    const { result, publicConsoleApiHttpClient } = setup({ response: verdicts });

    result.current.mutate({ evidence: evidence() });

    await vi.waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(result.current.data).toEqual(verdicts);
    const [path, body] = publicConsoleApiHttpClient.post.mock.calls[0];
    expect(path).toBe(ATTESTATION_VALIDATE_PATH);
