...
const USABLE_CREDENTIALS: UseProviderCredentialsResult["details"] = {
  type: "mtls",
...
describe(DownloadAttestationEvidence.name, () => {
  ...
  it("renders nothing when provider credentials are not usable", () => {
    setup({ state: "active", group: TEE_GROUP, credentials: { ...USABLE_CREDENTIALS, value: null, usable: false } });
    expect(screen.queryByRole("button", { name: /Attestation evidence/ })).not.toBeInTheDocument();
  });
  ...
  function setup(input: { state: LeaseDto["state"]; group?: DeploymentGroup; credentials?: UseProviderCredentialsResult["details"] }) {
    ...
    render(
      <DownloadAttestationEvidence
        ...
        providerCredentials={input.credentials ?? USABLE_CREDENTIALS}
